...
<?php
  ...
  if (isset($pInfo) && is_object($pInfo)) {
    $cPath = tep_get_generated_category_path_ids($pInfo->products_id, 'product');
    $heading[] = array('text' => '<strong>' . $pInfo->products_name . '</strong>');

    $contents[] = array('align' => 'center', 'text' => tep_draw_button(IMAGE_EDIT, 'document', tep_href_link('categories.php', 'pID=' . $pInfo->products_id . ($cPath !== 0 ? '&cPath=' . $cPath : '') . '&action=new_product')));
