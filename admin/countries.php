  ...
  if (tep_not_null($action)) {
    switch ($action) {
      ...
      case 'save':
        ...
        tep_redirect(tep_href_link('countries.php', 'page=' . tep_get_url_page_value() . '&cID=' . $countries_id));
        break;
      case 'deleteconfirm':
        $countries_id = tep_db_prepare_input($_GET['cID']);
        ...
        tep_redirect(tep_href_link('countries.php', 'page=' . tep_get_url_page_value()));
        break;
    }
  }
...
<?php	
	if (empty($action)) {
?>
	<div class="float-right"><?php echo tep_draw_button(IMAGE_NEW_COUNTRY, 'plus', tep_href_link('countries.php', 'page=' . tep_get_url_page_value() . '&action=new')); ?></div>
<?php
	}
?>	
...
<?php
  ...
  while ($countries = tep_db_fetch_array($countries_query)) {
    ...
    if (isset($cInfo) && is_object($cInfo) && ($countries['countries_id'] == $cInfo->countries_id)) {
      echo '<tr id="defaultSelected" class="table-primary" onclick="document.location.href=\'' . tep_href_link('countries.php', 'page=' . tep_get_url_page_value() . '&cID=' . $cInfo->countries_id . '&action=edit') . '\'">' . "\n";
    } else {
      echo '<tr class="dataTableRow" onclick="document.location.href=\'' . tep_href_link('countries.php', 'page=' . tep_get_url_page_value() . '&cID=' . $countries['countries_id']) . '\'">' . "\n";
    }
?>
				...
				<td class="dataTableContent" align="right"><?php if (isset($cInfo) && is_object($cInfo) && ($countries['countries_id'] == $cInfo->countries_id) ) { echo tep_image('images/icon_arrow_right.gif', ''); } else { echo '<a href="' . tep_href_link('countries.php', 'page=' . tep_get_url_page_value() . '&cID=' . $countries['countries_id']) . '">' . tep_image('images/icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
			</tr>
<?php
  }
?>
...
<?php
  ...
  switch ($action) {
    case 'new':
      ...
      $contents = array('form' => tep_draw_form('countries', 'countries.php', 'page=' . tep_get_url_page_value() . '&action=insert'));
      ...
      $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_SAVE, 'disk', null, 'primary') . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('countries.php', 'page=' . tep_get_url_page_value())));
      break;
    case 'edit':
      $heading[] = array('text' => '<strong>' . TEXT_INFO_HEADING_EDIT_COUNTRY . '</strong>');

      $contents = array('form' => tep_draw_form('countries', 'countries.php', 'page=' . tep_get_url_page_value() . '&cID=' . $cInfo->countries_id . '&action=save'));
      ...
      $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_SAVE, 'disk', null, 'primary') . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('countries.php', 'page=' . tep_get_url_page_value() . '&cID=' . $cInfo->countries_id)));
      break;
    case 'delete':
      $heading[] = array('text' => '<strong>' . TEXT_INFO_HEADING_DELETE_COUNTRY . '</strong>');

      $contents = array('form' => tep_draw_form('countries', 'countries.php', 'page=' . tep_get_url_page_value() . '&cID=' . $cInfo->countries_id . '&action=deleteconfirm'));
      $contents[] = array('text' => TEXT_INFO_DELETE_INTRO);
      $contents[] = array('text' => '<br /><strong>' . $cInfo->countries_name . '</strong>');
      $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_DELETE, 'trash', null, 'primary') . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('countries.php', 'page=' . tep_get_url_page_value() . '&cID=' . $cInfo->countries_id)));
      break;
    default:
      if (is_object($cInfo)) {
        $heading[] = array('text' => '<strong>' . $cInfo->countries_name . '</strong>');

        $contents[] = array('align' => 'center', 'text' => tep_draw_button(IMAGE_EDIT, 'document', tep_href_link('countries.php', 'page=' . tep_get_url_page_value() . '&cID=' . $cInfo->countries_id . '&action=edit')) . tep_draw_button(IMAGE_DELETE, 'trash', tep_href_link('countries.php', 'page=' . tep_get_url_page_value() . '&cID=' . $cInfo->countries_id . '&action=delete')));
