  ...
  if (tep_not_null($action)) {
    switch ($action) {
      ...
      case 'save':
        ...
        tep_redirect(tep_href_link('manufacturers.php', (isset($_GET['page']) ? 'page=' . tep_get_url_page_value() . '&' : '') . 'mID=' . $manufacturers_id));
        break;
      case 'deleteconfirm':
        ...
        tep_redirect(tep_href_link('manufacturers.php', 'page=' . tep_get_url_page_value()));
        break;
    }
  }
...
<?php	
	if (empty($action)) {
?>
	<div class="float-right"><?php echo tep_draw_button(IMAGE_INSERT, 'plus', tep_href_link('manufacturers.php', 'page=' . tep_get_url_page_value() . '&action=new')); ?></div>
<?php
	}
?>	
...
<?php
	...
	while ($manufacturers = tep_db_fetch_array($manufacturers_query)) {
		...
		if (isset($mInfo) && is_object($mInfo) && ($manufacturers['manufacturers_id'] == $mInfo->manufacturers_id)) {
		  echo '<tr id="defaultSelected" class="table-primary" onclick="document.location.href=\'' . tep_href_link('manufacturers.php', 'page=' . tep_get_url_page_value() . '&mID=' . $manufacturers['manufacturers_id'] . '&action=edit') . '\'">' . "\n";
		} else {
		  echo '<tr class="dataTableRow" onclick="document.location.href=\'' . tep_href_link('manufacturers.php', 'page=' . tep_get_url_page_value() . '&mID=' . $manufacturers['manufacturers_id']) . '\'">' . "\n";
		}
?>
				<td class="dataTableContent"><?php echo $manufacturers['manufacturers_name']; ?></td>
				<td class="dataTableContent"><?php if (isset($mInfo) && is_object($mInfo) && ($manufacturers['manufacturers_id'] == $mInfo->manufacturers_id)) { echo tep_image('images/icon_arrow_right.gif'); } else { echo '<a href="' . tep_href_link('manufacturers.php', 'page=' . tep_get_url_page_value() . '&mID=' . $manufacturers['manufacturers_id']) . '">' . tep_image('images/icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
			</tr>
<?php
	}
?>
...
<?php
  ...
  switch ($action) {
    case 'new':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_SAVE, 'disk', null, 'primary') . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('manufacturers.php', 'page=' . tep_get_url_page_value())));
      break;
    case 'edit':
      $heading[] = array('text' => '<strong>' . TEXT_HEADING_EDIT_MANUFACTURER . '</strong>');

      $contents = array('form' => tep_draw_form('manufacturers', 'manufacturers.php', 'page=' . tep_get_url_page_value() . '&mID=' . $mInfo->manufacturers_id . '&action=save', 'post', 'enctype="multipart/form-data"'));
      ...
      $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_SAVE, 'disk', null, 'primary') . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('manufacturers.php', 'page=' . tep_get_url_page_value() . '&mID=' . $mInfo->manufacturers_id)));
      break;
    case 'delete':
      $heading[] = array('text' => '<strong>' . TEXT_HEADING_DELETE_MANUFACTURER . '</strong>');

      $contents = array('form' => tep_draw_form('manufacturers', 'manufacturers.php', 'page=' . tep_get_url_page_value() . '&mID=' . $mInfo->manufacturers_id . '&action=deleteconfirm'));
      ...
      $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_DELETE, 'trash', null, 'primary') . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('manufacturers.php', 'page=' . tep_get_url_page_value() . '&mID=' . $mInfo->manufacturers_id)));
      break;
    default:
	  ...
	  if (isset($mInfo) && is_object($mInfo)) {
        ...
        $contents[] = array('align' => 'center', 'text' => tep_draw_button(IMAGE_EDIT, 'document', tep_href_link('manufacturers.php', 'page=' . tep_get_url_page_value() . '&mID=' . $mInfo->manufacturers_id . '&action=edit')) . tep_draw_button(IMAGE_DELETE, 'trash', tep_href_link('manufacturers.php', 'page=' . tep_get_url_page_value() . '&mID=' . $mInfo->manufacturers_id . '&action=delete')));
