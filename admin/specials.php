  ...
  if (tep_not_null($action)) {
    switch ($action) {
      case 'setflag':
        ...
        tep_redirect(tep_href_link('specials.php', 'page=' . tep_get_url_page_value() . 'sID=' . $_GET['id']));
        break;
      case 'insert':
        ...
        tep_redirect(tep_href_link('specials.php', 'page=' . tep_get_url_page_value()));
        break;
      case 'update':
        ...
        tep_redirect(tep_href_link('specials.php', 'page=' . tep_get_url_page_value() . '&sID=' . $specials_id));
        break;
      case 'deleteconfirm':
...
$('#expdate').datepicker({
  dateFormat: 'yy-mm-dd'
			...
			<nav>
				<ul class="float-right"><?php echo tep_draw_button(IMAGE_SAVE, 'disk', null, 'primary') . ' ' . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('specials.php', 'page=' . tep_get_url_page_value() . (isset($_GET['sID']) ? '&sID=' . $_GET['sID'] : ''))); ?></ul>
			</nav>
		</form>
	</div>
<?php
...
<?php
    ...
    while ($specials = tep_db_fetch_array($specials_query)) {
      ...
      if (isset($sInfo) && is_object($sInfo) && ($specials['specials_id'] == $sInfo->specials_id)) {
        echo '<tr id="defaultSelected" class="table-primary" onclick="document.location.href=\'' . tep_href_link('specials.php', 'page=' . tep_get_url_page_value() . '&sID=' . $sInfo->specials_id . '&action=edit') . '\'">' . "\n";
      } else {
        echo '<tr class="dataTableRow" onclick="document.location.href=\'' . tep_href_link('specials.php', 'page=' . tep_get_url_page_value() . '&sID=' . $specials['specials_id']) . '\'">' . "\n";
      }
?>
...
?>
                ...
                <td class="dataTableContent" align="right"><?php if (isset($sInfo) && is_object($sInfo) && ($specials['specials_id'] == $sInfo->specials_id)) { echo tep_image('images/icon_arrow_right.gif', ''); } else { echo '<a href="' . tep_href_link('specials.php', 'page=' . tep_get_url_page_value() . '&sID=' . $specials['specials_id']) . '">' . tep_image('images/icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
			</tr>
<?php
    }
?>
...
<?php
  ...
  switch ($action) {
    case 'delete':
      ...
      $contents = array('form' => tep_draw_form('specials', 'specials.php', 'page=' . tep_get_url_page_value() . '&sID=' . $sInfo->specials_id . '&action=deleteconfirm'));
      $contents[] = array('text' => TEXT_INFO_DELETE_INTRO);
      $contents[] = array('text' => '<br /><strong>' . $sInfo->products_name . '</strong>');
      $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_DELETE, 'trash', null, 'primary') . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('specials.php', 'page=' . tep_get_url_page_value() . '&sID=' . $sInfo->specials_id)));
      break;
    default:
      if (is_object($sInfo)) {
        $heading[] = array('text' => '<strong>' . $sInfo->products_name . '</strong>');

        $contents[] = array('align' => 'center', 'text' => tep_draw_button(IMAGE_EDIT, 'document', tep_href_link('specials.php', 'page=' . tep_get_url_page_value() . '&sID=' . $sInfo->specials_id . '&action=edit')) . tep_draw_button(IMAGE_DELETE, 'trash', tep_href_link('specials.php', 'page=' . tep_get_url_page_value() . '&sID=' . $sInfo->specials_id . '&action=delete')));
