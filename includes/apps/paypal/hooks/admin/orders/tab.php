    ...
    function execute()
    {
        ...
        if (tep_db_num_rows($ppstatus_query)) {
            ...
            if (isset($status['Transaction ID'])) {
                ...
                $info_button = $this->_app->drawButton($this->_app->getDef('button_details'), tep_href_link('orders.php', 'page=' . tep_get_url_page_value() . '&oID=' . $oID . '&action=edit&tabaction=getTransactionDetails'), 'primary', null, true);
        ...
        if (($status['Pending Reason'] == 'authorization') || ($status['Payment Status'] == 'In-Progress')) {
            ...
            if (!tep_db_num_rows($v_query)) {
                ...
                if ($capture_total > 0) {
                    ...
                    $capture_link = tep_href_link('orders.php', 'page=' . tep_get_url_page_value() . '&oID=' . $order['orders_id'] . '&action=edit&tabaction=doCapture');
        ...
        if ($status['Pending Reason'] == 'authorization') {
            ...
            if (!tep_db_num_rows($v_query)) {
                ...
                if ($capture_total > 0) {
                    ...
                    $void_link = tep_href_link('orders.php', 'page=' . tep_get_url_page_value() . '&oID=' . $order['orders_id'] . '&action=edit&tabaction=doVoid');
...
EOD;
    ...
    function getRefundButton($status, $order)
    {
        ...
        if ($can_refund === true) {
            ...
            $refund_link = tep_href_link('orders.php', 'page=' . tep_get_url_page_value() . '&oID=' . $_GET['oID'] . '&action=edit&tabaction=refundTransaction');
